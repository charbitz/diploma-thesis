...
def main(args):
    ...
    for index, row in tqdm(data_frame.iterrows(), total=len(data_frame)):
        ...
        for slice_n in range(len(view_files)):
            ...
            # if this is a right view flipped it back (horizontally) to the initial laterality:
            if "R" in view.upper():
                slice_image = np.fliplr(slice_image).copy()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--images",
        type=str,
        default="/mnt/seagate/DBT/TomoImages_Normal",
        help="root folder with preprocessed images",
    )
    parser.add_argument(
        "--output",
        type=str,
        default="/mnt/seagate/DBT/TomoImagesPP_Normal",
    ...
    parser.add_argument(
        ...
        default=True,
        action="store_true",
        help="flag to run preprocessing only on normal cases",
    )
    parser.add_argument(
