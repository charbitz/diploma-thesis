...
class TomoDetectionDataset(Dataset):
    ...
    def __init__(
        ...
        :param only_normal: filters only cases from normal group
        ...
        :dataframe_subset: the corresponding subset dataframe
        """
        # Put "all" subset in order all the desired volumes dataset (biopsy or normal) to be processed together
    ...
    def __getitem__(self, idx):
        ...
        if np.max(img) > 0:
            img = (img - np.min(img)).astype(np.float32) / (np.max(img) - np.min(img))
        else:
            # convert only to the datatype float32:
            img = img.astype(np.float32)
            print("min of image:", np.min(img))
            print("max of image:", np.max(img))
    ...
    def _df2dict(self, df_view_boxes):
        ...
        # here the coordinates of upper left point are
        # converted to the coordinates of the box center point:
